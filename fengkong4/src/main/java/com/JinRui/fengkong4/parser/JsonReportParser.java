...
@Slf4j
@Component
public class JsonReportParser implements ReportParser {
    
    private final ObjectMapper objectMapper = new ObjectMapper();
    
    @Override
    public TreeNode parse(String reportData, String format) throws ParseException {
        log.info("开始解析JSON格式征信报文，数据长度：{}", reportData.length());
        ...
        try {
            ...
            log.info("JSON格式征信报文解析完成，节点总数：{}", rootNode.getNodeCount());
            return rootNode;
            
        } catch (Exception e) {
            log.error("解析JSON格式征信报文失败", e);
